...
def GetOpts():
    ...
    parser.add_argument('-c', '--cpl_only', dest='cpl_only', action="store_true", help='Only do CPL file conversion (skip BOM)')
...
def main():
    ...
    for dir_name, subdir_list, file_list in os.walk(opts.project_dir):
        ...
        if (opts.cpl_only or netlist_path) and cpl_path:
            break

    if (not opts.cpl_only) and netlist_path is None:
    ...
    if not opts.cpl_only:
        bom_ok = GenerateBOM(netlist_path, bom_output_path, opts)
        if bom_ok:
    ...
    cpl_ok = FixRotations(cpl_path, cpl_output_path, db)
    if cpl_ok:
